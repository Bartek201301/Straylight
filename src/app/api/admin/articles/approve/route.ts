...
async function approveArticles(
  ...
  try {
    ...
    if (fetchError) {
      console.error('Failed to fetch articles for approval:', fetchError);
      return NextResponse.json(
        {
          error: 'Failed to retrieve articles for approval',
          code: 'FETCH_ERROR',
    ...
    // Security check: Only allow approval of pending articles
    ...
    if (pendingArticles.length === 0) {
      ...
      return NextResponse.json(
        {
          error: 'No pending articles found to approve',
          code: 'INVALID_STATUS',
          timestamp: new Date().toISOString(),
        },
        { status: 400 }
    ...
    if (updateError) {
      console.error('Failed to approve articles:', updateError);
      ...
      return NextResponse.json(
        {
          error: 'Failed to approve articles. Please try again.',
          code: 'UPDATE_ERROR',
          timestamp: new Date().toISOString(),
        },
        { status: 500 }
    ...
    const response = {
      success: true,
      approved: validArticleIds.length,
      ...
      message:
        nonPendingCount > 0
          ? `Approved ${validArticleIds.length} articles. ${nonPendingCount} articles were skipped (not in pending status).`
          : `Successfully approved ${validArticleIds.length} article${validArticleIds.length > 1 ? 's' : ''}.`,
      timestamp: new Date().toISOString(),
    };

    console.log(
      `Admin ${context.user.email} approved ${validArticleIds.length} articles`,
      {
    ...
    console.error('Article approval endpoint error:', error);
    ...
    return NextResponse.json(
      {
        error: 'Internal server error occurred during article approval',
        code: 'INTERNAL_ERROR',
        timestamp: new Date().toISOString(),
      },
      { status: 500 }
...
export const POST = withAdminAPIAuth(approveArticles);
